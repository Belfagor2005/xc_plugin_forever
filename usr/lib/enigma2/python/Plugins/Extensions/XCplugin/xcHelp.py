...
class xc_help(Screen):
	...
	def check_vers(self):
		...
		try:
			# Decode the URL and download the page
			decoded_url = Utils.b64decoder(installer_url)
			req = Request(decoded_url, headers={"User-Agent": "Mozilla/5.0"})
			page = urlopen(req).read().decode("utf-8")
			print("[INFO] Downloaded update info from: " + decoded_url)
		except Exception as e:
			print("[ERROR] Unable to fetch version info: " + str(e))
			return

		if page:
			version_line = next((line for line in page.split("\n") if line.startswith("version=")), None)
			changelog_line = next((line for line in page.split("\n") if line.startswith("changelog=")), None)

			if version_line:
				try:
					remote_version = version_line.split("'")[1]
				except:
					remote_version = version_line.split("=")[1].strip().strip("'").strip('"')

			if changelog_line:
				try:
					remote_changelog = changelog_line.split("'")[1]
				except:
					remote_changelog = changelog_line.split("=")[1].strip().strip("'").strip('"')

		self.new_version = remote_version
		self.new_changelog = remote_changelog

		print("[DEBUG] Local version: " + currversion + ", Remote version: " + self.new_version)

		try:
			local_tuple = tuple(map(int, currversion.split(".")))
			remote_tuple = tuple(map(int, self.new_version.split(".")))

			if local_tuple < remote_tuple:
				self.Update = True
				self.show_update_message()
				print("[INFO] Update available: " + currversion + " -> " + self.new_version)
			else:
				print("[INFO] Plugin is up to date")

		except Exception as e:
			print("[ERROR] Version comparison failed: " + str(e))
			if currversion < self.new_version:
				self.Update = True
				self.show_update_message()
				print("[FALLBACK] Update available: " + currversion + " -> " + self.new_version)
