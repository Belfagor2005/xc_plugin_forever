...
cfg.user = ConfigText(
    ...
    visible_width=50,
    fixed_size=False)
cfg.passw = ConfigPassword(
    default="******",
    visible_width=50,
    fixed_size=False,
    censor="*")
cfg.uptimezone = ConfigSelectionNumber(default=0, min=-20, max=80, stepwidth=1)
cfg.pdownmovie = ConfigSelection(
    default="JobManager", choices=[
        "JobManager", "Direct", "Requests"])
...
cfg.typelist = ConfigSelection(
    default="Multi Live & VOD",
    choices=[
        "Multi Live & VOD",
        "Multi Live/Single VOD",
        "Combined Live/VOD"])
cfg.typem3utv = ConfigSelection(
    default="MPEGTS to TV", choices=[
        "M3U to TV", "MPEGTS to TV"])
cfg.updateinterval = ConfigSelectionNumber(
    default=24, min=1, max=48, stepwidth=1)
...
def update_globals_dynamic():
    for var_name in globalsxp.__dict__.keys(
    ):  # Itera su tutte le chiavi di globalsxp
        if hasattr(
                cfg,
                var_name):  # Verifica se esiste una configurazione con lo stesso nome
            # Ottieni il valore dalla configurazione
            config_value = getattr(cfg, var_name).value
            current_value = getattr(globalsxp, var_name)
            if current_value != config_value:
                # Aggiorna la variabile globale
                setattr(globalsxp, var_name, config_value)
...
class xc_config(Screen, ConfigListScreen):
    def __init__(self, session):
        ...
        self["play_list"] = Label("Xstream Code Setup")
                ...
                if file_exists(iptvsh) and stat(iptvsh).st_size > 0:
                    try:
                        ...
                        if matches:
                            ...
                            self.session.open(
                                MessageBox, _(
                                    "Invalid format in %s. Could not extract server details." %
                                    iptvsh), MessageBox.TYPE_ERROR, timeout=5)

                    except Exception as e:
                        self.session.open(
                            MessageBox, _(
                                "Error reading or processing %s: %s" %
                                (iptvsh, str(e))), MessageBox.TYPE_ERROR, timeout=5)
                else:
                    self.session.open(
                        MessageBox,
            ...
            if answer is None:
                ...
                if file_exists(xc_list) and stat(xc_list).st_size > 0:
                    try:
                        ...
                        # Check if the file has enough lines and that they are
                        # not empty
                        if len(lines) < 4 or any(not line.strip()
                                                 for line in lines):
                            self.session.open(
                                MessageBox, _(
                                    "Invalid file format: not enough or empty lines in %s" %
                                    xc_list), MessageBox.TYPE_ERROR, timeout=5)
                            return
                        ...
                        self.session.open(
                            MessageBox, _(
                                "Error reading or processing the file: %s" %
    ...
    def createSetup(self):
        ...
        self.list.append(
            getConfigListEntry(
                _("Data Server Configuration:"),
                cfg.data,
                (_("Your Server Login and data input"))))
            ...
            self.list.append(
                getConfigListEntry(
                    indent + (
                        _("Server URL")),
                    cfg.hostaddress,
                    (_("Enter Server Url without 'http://' your_domine"))))
            self.list.append(getConfigListEntry(
                indent + (_("Server PORT")), cfg.port, (_("Enter Server Port Eg.:'8080'"))))
            self.list.append(getConfigListEntry(
                indent + (_("Server Username")), cfg.user, (_("Enter Username"))))
            self.list.append(getConfigListEntry(
                indent + (_("Server Password")), cfg.passw, (_("Enter Password"))))
        self.list.append(
            getConfigListEntry(
                _("Server Timeout"),
                cfg.timeout,
                (_("Timeout Server (sec)"))))
        self.list.append(
            getConfigListEntry(
                _("Adjust Timezone"),
                cfg.uptimezone,
                (_("Adjust Timezone (hour)"))))

        self.list.append(
            getConfigListEntry(
                _("Folder user file .xml"),
                cfg.pthxmlfile,
                (_("Configure folder containing .xml files\nPress 'OK' to change location."))))
        self.list.append(
            getConfigListEntry(
                _("Media Folder "),
                cfg.pthmovie,
                (_("Configure folder containing movie/media files\nPress 'OK' to change location."))))

        self.list.append(
            getConfigListEntry(
                _("Main Screen XL"),
                cfg.screenxl,
                (_("Active Main Screen Large"))))
        self.list.append(
            getConfigListEntry(
                _("LivePlayer Active "),
                cfg.LivePlayer,
                (_("Live Player for Stream .ts: set No for Record Live"))))
        if cfg.LivePlayer.value is True:
            self.list.append(
                getConfigListEntry(
                    indent + (
                        _("Live Services Type")),
                    cfg.live,
                    (_("Configure service Reference Dvb-Iptv-Gstreamer-Exteplayer3"))))

        self.list.append(
            getConfigListEntry(
                _("Download Type "),
                cfg.pdownmovie,
                (_("Configure type of download movie: JobManager/Direct."))))
        ...
        self.list.append(
            getConfigListEntry(
                _("Bouquet style "),
                cfg.typelist,
                (_("Configure the type of conversion in the favorite list"))))
        if cfg.typelist.value == "Combined Live/VOD":
            self.list.append(
                getConfigListEntry(
                    indent + (
                        _("Conversion type Output ")),
                    cfg.typem3utv,
                    (_("Configure type of stream to be downloaded by conversion"))))
        ...
        self.list.append(
            getConfigListEntry(
                _("Stop Player on Exit"),
                cfg.stoplayer,
                (_("If player active STOP player riproduction on exit"))))

        self.list.append(
            getConfigListEntry(
                _("Name Bouquet Configuration:"),
                cfg.infoexp,
                (_("Set Name for MakerBouquet"))))
        if cfg.infoexp.getValue():
            self.list.append(
                getConfigListEntry(
                    indent + (
                        _("Name Bouquet Export")),
                    cfg.infoname,
                    (_("Configure name of exported bouquet. Default is myBouquet"))))

        self.list.append(
            getConfigListEntry(
                _("Place IPTV bouquets at "),
                cfg.bouquettop,
                (_("Configure to place the bouquets of the converted lists"))))

        self.list.append(
            getConfigListEntry(
                _("Automatic bouquet update (schedule):"),
                cfg.autobouquetupdate,
                (_("Active Automatic Bouquet Update"))))
        if cfg.autobouquetupdate.value is True:
            self.list.append(getConfigListEntry(indent + (_("Schedule type:")),
                             cfg.timetype, (_("At an interval of hours or at a fixed time"))))
            if cfg.timetype.value == "interval":
                self.list.append(
                    getConfigListEntry(
                        ...
                        cfg.updateinterval,
                        (_("Configure every interval of hours from now"))))
            if cfg.timetype.value == "fixed time":
                self.list.append(
                    getConfigListEntry(
                        2 * indent + (
                            _("Time to start update:")),
                        cfg.fixedtime,
        ...
        if cfg.picons.value:
            self.list.append(
                getConfigListEntry(
                    indent + (
                        _("Picons IPTV bouquets to ")),
                    cfg.pthpicon,
                    (_("Configure folder containing picons files\nPress 'OK' to change location."))))
        self.list.append(
            getConfigListEntry(
                _("Link in Main Menu "),
                cfg.strtmain,
                (_("Display XCplugin in Main Menu"))))
        ...
        if cfg.picons.value:
            ...
            if cfg.pthxmlfile.value:
                if not file_exists(cfg.pthxmlfile.value):
                    try:
                        with open(cfg.pthxmlfile.value, 'w') as f:
                            f.write("")
                        print("Created missing file", cfg.pthxmlfile.value)
                    except Exception as e:
                        self.session.open(
                            MessageBox, _(
                                "%s NOT DETECTED and could not be created!\nError: %s" %
                                (cfg.pthxmlfile.value, str(e))), MessageBox.TYPE_ERROR, timeout=5)
                        return
        ...
        try:
            if str(cfg.hostaddress.value) != 'exampleserver.com':
                if file_exists(Path_XML + '/xclink.txt'):
                    linecode = 'http://' + str(cfg.hostaddress.value) + ':' + str(cfg.port.value) + '/get.php?username=' + str(
                        cfg.user.value) + '&password=' + str(cfg.passw.value) + '&type=m3u_plus'
                    line_exists = False
                    with codecs.open(Path_XML + '/xclink.txt', "r+", encoding="utf-8") as f:
                        ...
                        if not line_exists:
                            ...
                            self.session.open(
                                MessageBox,
                                _("Line appended to playlist"),
                                type=MessageBox.TYPE_INFO,
                                timeout=5)
                        else:
                            self.session.open(
                                MessageBox,
                                _("Line already exists in playlist"),
                                type=MessageBox.TYPE_INFO,
                                timeout=5)
        except Exception as e:
