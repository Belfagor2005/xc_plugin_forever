...
cfg.pdownmovie = ConfigSelection(default="JobManager", choices=["JobManager", "Direct"])
			...
			if cfg.pthxmlfile.value:
				if not file_exists(cfg.pthxmlfile.value):
					try:
						...
						self.session.open(
							MessageBox,
							_("%s NOT DETECTED and could not be created!\nError: %s" % (cfg.pthxmlfile.value, str(e))),
							MessageBox.TYPE_ERROR,
							timeout=5
						)
						return
