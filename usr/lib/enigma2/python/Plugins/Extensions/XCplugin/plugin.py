...
from Components.config import config, ConfigSelection, getConfigListEntry, NoSave, ConfigText, ConfigDirectory, ConfigNumber
from Plugins.Plugin import PluginDescriptor
from Screens.Console import Console
from Screens.InfoBar import MoviePlayer, InfoBar
from Screens.InfoBarGenerics import *
from Screens.InfoBarGenerics import InfoBarShowHide, NumberZap, InfoBarSeek, InfoBarAudioSelection, InfoBarSubtitleSupport, InfoBarServiceNotifications, InfoBarMoviePlayerSummarySupport
...
from Tools.Directories import fileExists, copyfile, pathExists, resolveFilename, SCOPE_PLUGINS, SCOPE_LANGUAGE
...
from os import environ, listdir, path, readlink, system, rename, access, X_OK, chmod
...
config.plugins.XCplugin.port                      = ConfigNumber(default = 8888)
...
def check_port(tport):
    ...
    domain = ''
    port = ''
    if str(config.plugins.XCplugin.port.value) != '8888':
...
class iptv_streamse():
                ...
                for channel in xml.findall("channel"):
                    ...
                    if stream_url and "/live/" in stream_url:
                        ...
                        if len(name.split("[")) > 1:
                            ...
                            if description != '':
                                timematch = re.findall('\[(\d\d:\d\d)\]', description)
                                titlematch = re.findall('\[\d\d:\d\d\](.*)', description)
                                descriptionmatch = re.findall('\n(?s)\((.*?)\)', description)
                                if timematch:
                                    if len(timematch) > 0:
    ...
    def _request(self, url):
        if not "exampleserver.com" in config.plugins.XCplugin.hostaddress.value :
...
class xc_Main(Screen):
    ...
    def filterChannels(self, result):
        if result:
            ...
            if len(self.filter_search):
                ...
                # self.mlist.setList(map(channelEntryIPTVplaylist, iptv_list_tmp))
    ...
    def update_list(self):
        ...
        STREAMS.read_config()
        if not "exampleserver.com" in STREAMS.xtream_e2portal_url:
            STREAMS.get_list(STREAMS.xtream_e2portal_url)
            self.update_channellist()
        self.checkinf()
            ...
            try:
                if series == True:
                    ...
                    for name, url in match:
                        ...
                        name = name + ext
                        self.icount += 1
                        JobManager.AddJob(downloadJob(self, "wget -c '%s' -O '%s%s'" % (url, Path_Movies2, name), Path_Movies2 + name, name))
                    MemClean()
                else:
                        pmovies=False
                        self.mbox = self.session.open(MessageBox, _("Only Series Episodes Allowed!!!"), MessageBox.TYPE_INFO, timeout=5)
                ...
                if selected_channel[7] != "" and selected_channel[7] != "n/A" and selected_channel[7] != None:
                    if selected_channel[7].find("http") == -1:
                        ...
                        if selected_channel[7].startswith('https'):
                            ...
                            if os.path.exists(desc_image) == False or STREAMS.img_loader == False:
                                desc_image = "%s/%s.jpg" % (Path_Tmp, cover_md5)
        ...
        try:
            ...
            try:
                ...
                if selected_channel[3] != "" and selected_channel[3] != "n/A" and selected_channel[3] != None:
                    if not 'http' in selected_channel[3]:
                        self.decodeImage(piclogo)
                    else:
                        if STREAMS.img_loader == False:
                            m = hashlib.md5()
...
class xc_Play(Screen):
    ...
    def message3(self):
        if self.downloading == True:
            ...
            return
        elif not "exampleserver.com" in STREAMS.xtream_e2portal_url:
...
def Plugins(**kwargs):
    result = [PluginDescriptor(name="XCplugin Forever", description=version, where=[PluginDescriptor.WHERE_AUTOSTART, PluginDescriptor.WHERE_SESSIONSTART], fnc=autostart, wakeupfnc=get_next_wakeup), PluginDescriptor(name="XCplugin", description=version, where=PluginDescriptor.WHERE_PLUGINMENU, icon=iconpic, fnc=main)]       #fnc=Start_iptv_player)]
...
def createCfg_xml():
        ...
        with open(configfile, 'w') as f:
            f.write('<config>\r\n' +
                    ...
                    '        <name>' + infoname + '</name>\r\n' +
                    '        <enabled>1</enabled>\r\n' +
                    '        <m3uurl><![CDATA[' + m3u_url + '&type=m3u_plus&output=ts' + ']]></m3uurl>\r\n' +
                    '        <epgurl><![CDATA[' + epg_url + ']]></epgurl>\r\n' +
                    '        <username><![CDATA['+ username + ']]></username>\r\n' +
                    '        <password><![CDATA['+ password + ']]></password>\r\n' +
                    '        <iptvtypes>' + iptv_types + '</iptvtypes>\r\n' +
                    '        <streamtypetv>' + streamtype_tv + '</streamtypetv>\r\n' +
                    '        <streamtypevod>' + streamtype_vod + '</streamtypevod>\r\n' +
                    '        <multivod>' + multi_vod + '</multivod>\r\n' +
                    '        <allbouquet>' + all_bouquet + '</allbouquet>\r\n' +
                    '        <picons>' + picons +'</picons>\r\n' +
                    '        <iconpath>' + Path_Picons + '</iconpath>\r\n' +
                    '        <xcludesref>0</xcludesref>\r\n' +
                    '        <bouqueturl></bouqueturl>\r\n' +
                    '        <bouquetdownload>0</bouquetdownload>\r\n' +
                    '        <bouquettop>' + bouquet_top + '</bouquettop>\r\n' +
                    '  </supplier>\r\n' +
                    '</config>\r\n')
            f.close
