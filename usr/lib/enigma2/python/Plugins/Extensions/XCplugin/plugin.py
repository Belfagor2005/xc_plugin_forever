...
class xc_home(Screen):
    ...
    def Team(self):
        ...
        self.session.openWithCallback(self.OpenList, OpenServer)

    def helpx(self):
        self.session.openWithCallback(self.start, xc_help)

    def taskManager(self):
        self.session.openWithCallback(self.start, xc_StreamTasks)
...
def Plugins(**kwargs):
    result = [PluginDescriptor(name="XCplugin Forever", description=version, where=[PluginDescriptor.WHERE_AUTOSTART, PluginDescriptor.WHERE_SESSIONSTART], fnc=autostart, wakeupfnc=get_next_wakeup),
              PluginDescriptor(name="XCplugin", description=version, where=PluginDescriptor.WHERE_PLUGINMENU, icon=iconpic, fnc=main)]
