...
class xc_StreamTasks(Screen):
	...
	def keyOK(self):
		...
		if job == "movie" and len(current) > 2 and current[2] == "Finished":
			...
			self.session.open(MessageBox, _("Invalid or unsupported job type"), MessageBox.TYPE_INFO, timeout=5)
...
class downloadTask(Task):
	...
	def afterRun(self):
		if self.returncode != 0:
			...
			self.lasterrormsg = _("Filesystem error: Cannot write to destination")
