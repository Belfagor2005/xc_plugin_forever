...
def u8(s):
    return unicode(s, "utf-8") if isinstance(s, bytes) else unicode(s)
...
def _s(text):
    """Safe string conversion for Python 2 and 3"""
    if text is None:
        return u''
    ...
    try:
        ...
        try:
            return unicode(str(text), 'utf-8', errors='ignore')
        except BaseException:
            return u''
...
class xc_Playlist(Screen):
    def __init__(self, session, STREAMS):
        ...
        try:
            ...
            self.gui_update_timer_conn = self.gui_update_timer.timeout.connect(
                self._safe_update)
        ...
        self["actions"] = HelpableActionMap(self, "XCpluginActions", {
            ...
            "info": self.xc_Help,
            "help": self.xc_Help}, -1)
        self.onShown.append(self.enableKeys)
        self.onHide.append(self.disableKeys)
    ...
    def selOn(self, host, port, username, password):
        ...
        try:
            ...
            globalsxp.urlinfo = "http://" + host + ":" + \
                str(port) + "/player_api.php?username=" + username + "&password=" + password
            response = self.session_http.get(globalsxp.urlinfo, timeout=5)
            if response.status_code == 404:
                return "N/A", "API Not Found"
    ...
    def openList(self):
        ...
        m3ulistxc([str(name) for name in self.names], self["list"])
    ...
    def verify_servers(self):
        ...
        try:
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = {}
                for idx, (host, port, username,
                          password) in enumerate(self.entries):
                    if not self.active:
                        break
                    future = executor.submit(
                        self.selOn, host, port, username, password)
                    futures[future] = idx
    ...
    def schedule_gui_update(self, immediate=False):
        """Schedule a batch GUI update from the main thread"""
            ...
            if matchx:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if "user_info" in y:
                            if y["user_info"].get("auth") == 1:
                                status = y["user_info"].get("status", "N/A")
                                created_at = y["user_info"].get(
                                    "created_at", "N/A")
                                exp_date = y["user_info"].get(
                                    "exp_date", "N/A")
                                active_cons = y["user_info"].get(
                                    "active_cons", "N/A")
                                max_connections = y["user_info"].get(
                                    "max_connections", "N/A")
                                server_protocol = y["server_info"].get(
                                    "server_protocol", "N/A")
                                time_now = y["server_info"].get(
                                    "time_now", "N/A")
                                time_zone = y["server_info"].get(
                                    "timezone", "N/A")
                                ...
                                created_at = (
                                    strftime(
                                        TIME_GMT, gmtime(
                                            int(created_at))) if created_at and created_at.isdigit() else created_at)
                                exp_date = (
                                    strftime(
                                        TIME_GMT, gmtime(
                                            int(exp_date))) if exp_date and exp_date.isdigit() else exp_date)
                    ...
                    if response.status_code == 401:
                        ...
                        message = "HTTP Error " + \
                            str(response.status_code) + " for user " + username
