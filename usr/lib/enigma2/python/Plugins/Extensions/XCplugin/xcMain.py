...
class xc_Main(Screen):
        ...
        try:
            ...
            try:
                ...
                if self.stars_empty_pixmap is None:
                    ...
                    print(
                        f"[RATING] Empty pixmap loaded: {'success' if self.stars_empty_pixmap else 'failed'}")

                if self.stars_filled_pixmap is None:
                    self.stars_filled_pixmap = LoadPixmap(filled_path)
                    print(
                        f"[RATING] Filled pixmap loaded: {'success' if self.stars_filled_pixmap else 'failed'}")
        ...
        if globalsxp.series is True and globalsxp.btnsearch == 1:
            if answer is None:
                ...
                if file_exists(
                    ...
                    self.session.openWithCallback(self.check_download_ser, MessageBox, _(
                            ...
                            for name, url in match:
                                if url.startswith('http'):
                                    ...
                                    cmd = "wget --no-cache --no-dns-cache -U '%s' -c '%s' -O '%s%s'" % (
                                        'Enigma2 - XC Forever Plugin', url, str(globalsxp.Path_Movies2), self.title)
                                    if "https" in str(url):
    ...
    def downloadx(self):
        cmd = "wget --no-cache --no-dns-cache -U '%s' -c '%s' -O '%s'" % (
            'Enigma2 XC Forever Plugin', str(self.vod_url), str(self.file_down))
        if "https" in str(self.vod_url):
            cmd = "wget --no-check-certificate --no-cache --no-dns-cache -U '%s' -c '%s' -O '%s'" % (
                'Enigma2 - XC Forever Plugin', str(self.vod_url), str(self.file_down))
        self.downloading = False
        try:
            JobManager.AddJob(
