						...
						try:
							...
							for name, url in match:
								if url.startswith('http'):
									...
									self.icount += 1
									cmd = "wget -U '%s' -c '%s' -O '%s%s'" % ('Enigma2 - XC Forever Plugin', url, str(globalsxp.Path_Movies2), self.title)
									if "https" in str(url):
										cmd = "wget --no-check-certificate -U '%s' -c '%s' -O '%s%s'" % ('Enigma2 - XC Forever Plugin', url, str(globalsxp.Path_Movies2), self.title)
