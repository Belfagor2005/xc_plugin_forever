...
class DownloadThread(threading.Thread):
	def __init__(self, url, target, progress_callback, completion_callback, error_callback):
		super().__init__()
		self.url = url
		self.target = target
		...
		try:
			...
			with open(self.target, 'wb') as f:
				while True:
					...
					self.progress_callback(downloaded, total_size, speed, remaining)
...
class imagedownloadScreen(Screen):
	...
	def updateProgress(self, current, total, speed, remaining):
		reactor.callFromThread(self._updateProgressGUI, current, total, speed, remaining)
