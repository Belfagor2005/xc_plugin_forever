...
class Provider():
    ...
    def _create_epgimport_source(self, sources, group = None):
        ...
        with open(os.path.join(EPGIMPORTPATH, source_filename), 'w+') as f:
            f.write('<sources>\n')
            f.write('{}<sourcecat sourcecatname="XC Forever">\n'.format(indent))
                ...
                for cat in self._category_order:
                    if cat in self._dictchannels:
                        if self._category_options[cat].get('type', 'live') == 'live':
                            ...
                            f.write('{}<category name="{}" nameOverride="{}" enabled="{}" customCategory="{}"/>\r\n'.format(2 * indent, xml_escape(cat).encode('utf-8'), xml_escape(cat_title_override).encode('utf-8'), str(self._category_options[cat].get('enabled', True)).lower(), str(self._category_options[cat].get('customCategory', False)).lower()))
                            ...
                            f.write('{}<category name="{}" nameOverride="{}" enabled="{}" />\r\n'.format(2 * indent, 'VOD', xml_escape(cat_title_override).encode('utf-8'), str(cat_enabled).lower()))
                ...
                for cat in self._category_order:
                    if cat in self._dictchannels:
                        if self._category_options[cat].get('type', 'live') == 'live':
                            ...
                            for x in self._dictchannels[cat]:
                                if not x['stream-name'].startswith('placeholder_'):
                                    f.write('{}<channel name="{}" nameOverride="{}" tvg-id="{}" enabled="{}" category="{}" categoryOverride="{}" serviceRef="{}" clearStreamUrl="{}" />\r\n'.format(2 * indent, xml_escape(x['stream-name'].encode('utf-8')), xml_escape(x.get('nameOverride', '').encode('utf-8')), xml_escape(x['tvg-id'].encode('utf-8')), str(x['enabled']).lower(), xml_escape(x['group-title'].encode('utf-8')), xml_escape(x.get('categoryOverride', '').encode('utf-8')), xml_escape(x['serviceRef']), 'false' if x['stream-url'] else 'true'))
                                else:
                                    f.write('{}<channel name="{}" category="{}" />\r\n'.format(2 * indent, 'placeholder', xml_escape(cat.encode('utf-8'))))
                ...
                if cat not in vod_categories or self.config.multi_vod:
                    with open(bouquet_filepath, 'w+') as f:
                        ...
                        if self._category_options[cat].get('type', 'live') == 'live':
                            ...
                            bouquet_name = '{} - {}'.format(self._category_options['VOD']['nameOverride'].decode('utf-8'), cat_title.replace('VOD - ', '').decode('utf-8'))
                        channel_num = 0
                            ...
                            for x in self._dictchannels[cat]:
                                if not x['stream-name'].startswith('placeholder_'):
                                    ...
                                    if x['enabled']:
                                        ...
                                        f.write('{}<channel id="{}">{}:http%3a//example.m3u8</channel> <!-- {} -->\n'.format(indent, xml_escape(tvg_id.encode('utf-8')), epg_service_ref, xml_safe_comment(xml_escape(get_service_title(x).encode('utf-8')))))
