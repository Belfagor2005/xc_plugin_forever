...
class xc_maker(Screen):
    ...
    def maker(self):
        if str(cfg.typelist.value) == "Multi Live & VOD":
            ...
            self.session.openWithCallback(
                self.createCfgxml, MessageBox, _("Convert Playlist to: %s ?") %
                dom, MessageBox.TYPE_YESNO, timeout=10)  # default=False)
            ...
            self.session.openWithCallback(
                self.save_tv, MessageBox, _("Convert Playlist to: %s ?") %
                dom, MessageBox.TYPE_YESNO, timeout=10)  # default=False)
        else:
            pass
    ...
    def remove(self, answer=None):
        if answer is None:
            self.session.openWithCallback(
                self.remove, MessageBox, _("Remove Playlist from Bouquets?"))
        elif answer:
            uninstaller()
            self.session.open(
                MessageBox,
...
def save_old():
    ...
    tag = "xc_"
    xc12 = globalsxp.urlinfo.replace(
        "enigma2.php", "get.php") + '&type=dreambox&output=mpegts'
    xc13 = globalsxp.urlinfo.replace(
        "enigma2.php", "get.php") + '&type=m3u_plus&output=ts'
    in_bouquets = False
    desk_tmp = xcname = ''
    try:
        if cfg.typem3utv.value == 'MPEGTS to TV':
            file_path = join(
                enigma_path, 'userbouquet.%s%s_.tv' %
                (tag, namebouquet))
                        ...
                        for line in infile:
                            if line.startswith(
                                ...
                                outfile.write('#SERVICE 4097:0:1:0:0:0:0:0:0:0:%s\r\n' % line.split(
                                    '[')[-1].split(']')[0].strip().replace(':', '%3a'))
                                outfile.write('#DESCRIPTION %s\r\n' % desk_tmp)
                            elif '<title>' in line:
                                if '<![CDATA[' in line:
                                    desk_tmp = '%s' % line.split(
                                        '[')[-1].split(']')[0].strip()
                                else:
                                    desk_tmp = '%s' % line.split(
                                        '<')[1].split('>')[1].strip()
            ...
            try:
                with open("/etc/enigma2/new_bouquets.tv", "w") as new_bouquet:
                    ...
                    if cfg.bouquettop.value == "Top":
                        ...
                        new_bouquet.writelines(
                            line for line in file_read if not line.startswith("#NAME"))
                    else:
...
def make_bouquet(session):
    if "exampleserver.com" not in globalsxp.STREAMS.xtream_e2portal_url:
        ...
        with open(configfilexml, 'w') as f:
            ...
            configtext += '\t\t<m3uurl><![CDATA[' + m3u_url + \
                '&type=m3u_plus&output=ts' + ']]></m3uurl>\r\n'
            configtext += '\t\t<epgurl><![CDATA[' + \
                epg_url + ']]></epgurl>\r\n'
            configtext += '\t\t<username><![CDATA[' + \
                username + ']]></username>\r\n'
            configtext += '\t\t<password><![CDATA[' + \
                password + ']]></password>\r\n'
        ...
        session.open(
            xcConsole, _("Conversion %s in progress: ") %
            dom, [
                "%s" %
                com], closeOnSuccess=True)
