...
class AspectManager:
	...
	def get_current_aspect(self):
		"""Return the current aspect ratio of the device."""
		try:
	...
	def restore_aspect(self):
		"""Restore the original aspect ratio when exiting the plugin."""
		try:
		...
		try:
			if globalsxp.STREAMS.trial != '':
				...
				try:
					...
					# Make sure the path ends with a slash for join to work properly
...
class xc_Play(Screen):
	...
	def message3(self, answer=None):
		if "exampleserver.com" not in globalsxp.STREAMS.xtream_e2portal_url:
			...
			if answer is None:
				...
				self.in_tmp = join(globalsxp.Path_Movies, self.name_m3u + ".m3u")
	...
	def showError(self, error):
		...
		self['progress'].setValue(0)
		self["progress"].hide()
		self.session.open(MessageBox, _('Download failed: ') + str(error), MessageBox.TYPE_WARNING)
		self.refreshmylist()
		...
		try:
			with open(self.name, "r", encoding="utf-8", errors="ignore") as f:
				...
				for line in f:
					...
					# Support for XML-like formats (without changing original logic)
					elif "<stream_url><![CDATA" in line:
			...
			self.session.open(MessageBox, "Error reading M3U file: " + str(e), MessageBox.TYPE_ERROR)
